...
// The lowest reading across a FULL window, or -1 until there is one.
//
// A leak never gives its descriptors back, so the lowest reading keeps climbing.
// Load raises the peak and lets it fall again, and one low reading anywhere in
// the window is enough to say so — which is why a partial window answers
// nothing rather than answering early.
int windowFloor(const QList<int> &samples, int window)
{
...
// How much growth counts as a leak. Scaled to the resource rather than a flat 64
// — on a machine that allows ten thousand descriptors, sixty-four of them is
// noise — but capped, because a limit of a million would put the threshold past
// any leak worth catching. A thousand descriptors that never come back is
// unambiguous either way.
int fdGrowthThreshold(int fdLimit)
{
    return std::clamp(fdLimit > 0 ? fdLimit / 4 : 256, 256, 1024);
}
...
void QtTrustTunnelClient::checkFdHealth() {
    ...
    const int floor = windowFloor(m_fdSamples, kFdWindow);
    if (m_fdBaseline >= 0 && floor >= 0 && floor - m_fdBaseline >= fdGrowthThreshold(fdLimit)) {
        forceFdReconnect(
                QStringLiteral("[fd watchdog] Open fds have not fallen below %1 across %2 checks, "
                               "%3 above the baseline of %4 (currently %5)")
                        .arg(floor)
                        .arg(kFdWindow)
                        .arg(floor - m_fdBaseline)
                        .arg(m_fdBaseline)
                        .arg(openFds),
